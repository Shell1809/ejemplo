...
foreach ($routes as $route => $config) {
    ...
    if ($match) {
        ...
        // ==================== CORRECCIÓN DE VALIDACIÓN DE MÉTODO HTTP ====================
        // Admite strings únicos ('POST') o arreglos con múltiples verbos (['GET', 'POST'])
        if (isset($config['method'])) {
            $metodosPermitidos = is_array($config['method']) ? $config['method'] : [$config['method']];
            
            if (!in_array($method, $metodosPermitidos)) {
                if ($isApiRoute || $isAjax) {
                    header('Content-Type: application/json');
                    http_response_code(405);
                    echo json_encode([
                        'success' => false,
                        'message' => 'Error 405: Método HTTP no permitido para esta acción.'
                    ]);
                    exit();
                } else {
                    http_response_code(405);
        ...
        if (isset($config['rol'])) {
            if (!isset($_SESSION['rol'])) {
                if ($isApiRoute || $isAjax) {
                    ...
                    echo json_encode(['success' => false, 'message' => 'Prohibido: Rol no definido']);
                    exit();
                } else {
                    redirect('login');
                }
            ...
            if (!verificarRol($config['rol'], $_SESSION['rol'])) {
                if ($isApiRoute || $isAjax) {
                    header('Content-Type: application/json');
                    http_response_code(403);
                    echo json_encode(['success' => false, 'message' => 'No tiene permisos necesarios']);
                    exit();
                } else {
                    redirect('login');
                }
        ...
        if (!method_exists($controller, $actionName)) {
            if ($isApiRoute || $isAjax) {
                header('Content-Type: application/json');
                http_response_code(500);
                echo json_encode(['success' => false, 'message' => "Acción '{$actionName}' no encontrada en {$controllerName}"]);
                exit();
            } else {
