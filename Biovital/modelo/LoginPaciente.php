...
class LoginPaciente {
    ...
    function Loguearse($cedula, $pass) {
        // Primero obtener el hash de la base de datos utilizando la cédula
        ...
        if ($usuario) {
            ...
            if (password_verify($pass, $usuario->password_hash)) {
                // Actualizamos el último acceso automáticamente al iniciar sesión con éxito
                $this->actualizarUltimoAcceso($usuario->id_paciente);
                
                $this->objetos = array($usuario);
                return $this->objetos; // Devuelve el usuario si todo está perfecto
            }
        }
        
        // Si no se encuentra el usuario o la contraseña no coincide, devuelve un arreglo vacío
        $this->objetos = array();
        return $this->objetos;
    }
